...
from config import DEFAULT_GROUP_ID, LOG_CHANNEL_ID, MIN_INTERVAL_BETWEEN_FILES, AVAILABLE_GROUPS
...
async def process_file(bot: Bot, message: types.Message, file_stream: BinaryIO, file_extension: str, group_id: int) -> None:
    """
    ...
    Args:
        ...
        group_id: ID of the group to send quizzes to
    """
    temp_path = None
    try:
        ...
        if file_extension == "csv":
            await process_csv_file(bot, message, temp_path, group_id)
        elif file_extension == "pdf":
            await process_pdf_file(bot, message, temp_path, group_id)
...
async def process_csv_file(bot: Bot, message: types.Message, file_path: str, group_id: int) -> None:
    """
...
async def process_pdf_file(bot: Bot, message: types.Message, file_path: str, group_id: int) -> None:
    """
...
async def handle_document(bot: Bot, message: types.Message):
    ...
    try:
        ...
        # Store file data and show group selection
        ...
        for group_name, group_id in AVAILABLE_GROUPS.items():
            builder.button(text=group_name, callback_data=f"group_{group_id}")
        
        # Send group selection message
        await message.reply(
            "🔍 تم استلام الملف بنجاح!\n"
            "الرجاء اختيار المجموعة التي تريد إرسال الأسئلة إليها:",
            reply_markup=builder.as_markup()
        )
        
        # Set user state to waiting for group selection
